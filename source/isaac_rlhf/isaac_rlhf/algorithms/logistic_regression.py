        ...
        for Xb, yb in train_loader:
            ...
            if epoch == epochs or epoch % 100 == 0:
                with torch.inference_mode():
                    ...
                    gt_bce_tot += F.binary_cross_entropy_with_logits(gt_logits, yb).item() / len(train_loader)
            
        # log to console
        if epoch == epochs or epoch % 100 == 0:
            train_acc = evaluate(train_loader)
            test_acc  = evaluate(test_loader)
            print(f"[MLE epoch {epoch}/{epochs}] bce={bce_tot:.4e}, gt_bce={gt_bce_tot:.4e}, L2={l2.item():.4e}"
